...
**jij** 是一个由 AI 驱动的操作系统智能代理，将自然语言指令转换为可控的系统操作，内置五级风险安全层，支持本地与 SSH 远程双模式。
...
若已安装 Claude Code 或 Codex CLI，启动 `jij chat` 时会自动检测其配置文件并询问是否导入 API Key，一键完成配置，无需手动填写。
...
jij 会自动检测已设置的 API Key 并选择对应 Provider，启动时显示检测结果。
...
```bash
jij chat --local                                           # 强制本地，跳过菜单
jij chat --no-prompt                                       # 脚本场景：完全不交互
jij chat --ssh root@192.168.1.100                          # 直接进入 SSH 远程
jij chat --ssh admin@host:2222 --ssh-key ~/.ssh/id_ed25519
```
...
cd agent-unix
cargo build --release
# 二进制文件：target/release/jij（Windows：jij.exe）
```
...
> 自动安装到 `~/.cargo/bin/`，Rust 工具链已将该目录加入 PATH，安装后可直接运行 `jij`。

或手动复制：

```bash
# macOS / Linux
sudo cp target/release/jij /usr/local/bin/
...
```powershell
# 编译后将 jij.exe 添加到 PATH，设置 API Key 后即可使用
$env:ANTHROPIC_API_KEY = "sk-ant-xxxxxxxx"
jij chat
# jij 会自动设置控制台为 UTF-8，中文显示正常
```
...
export AGENT_UNIX_LLM_BASE_URL=https://your-proxy.com
export AGENT_UNIX_LLM_MODEL=claude-opus-4-5
```

### 方式二：配置文件

```bash
jij config --setup              # 交互式向导（自动检测本地 AI 工具配置）
jij config --show               # 查看当前配置
jij config --list               # 列出所有支持的 Provider
jij config --delete             # 删除配置文件
```

配置文件路径：`~/.jij/config.json`（Windows: `%USERPROFILE%\.jij\config.json`）

### 方式三：CLI 参数（最高优先级）

```bash
jij --provider openai --model gpt-4.1 chat
...
```
CLI 参数 > 配置文件 > AGENT_UNIX_LLM_* 环境变量 > Provider 专属环境变量 > 默认值
```
...
jij chat --ssh root@192.168.1.100   # SSH 远程模式
jij run "查看磁盘使用情况"             # 单条指令执行
jij run --dry-run "清理30天前日志"     # 预览模式（不实际执行）
jij explain /etc/nginx/nginx.conf    # AI 解释配置文件
jij watch --duration 120            # 后台系统监控 120 秒
jij info                            # 显示系统信息与环境
jij config --setup                  # 配置 LLM Provider
jij playbooks                       # 列出所有 Playbook
```
...
jij --mode auto chat   # 自动模式（Medium 不询问）
```

审计日志路径：`~/.jij/audit-YYYYMMDD.jsonl`
...
Playbook 是可重复执行的操作序列。jij 内置 6 个常用模板：
...
用户自定义 Playbook 保存到 `~/.jij/playbooks/`，项目级 Playbook 保存到 `./.jij/playbooks/`。
...
```bash
jij explain /etc/nginx/nginx.conf
jij explain /var/log/auth.log
jij explain             # 不加参数则列出所有支持的文件类型
```
...
```bash
jij watch --duration 120   # 启动后台监控 120 秒
```
...
当没有配置 API Key 时，jij 会自动扫描以下 AI 工具的配置文件：
...
**流程**：扫描时仅检查文件是否存在（不读取内容） → 向用户展示路径 → **用户授权后**才读取并提取 Key → 脱敏显示（`sk-ant-***`）→ 保存到 `~/.jij/config.json`。

---

## 图片分析

在 iTerm2 等支持图片预览的终端中，可以直接将图片拖入或粘贴到输入框，jij 会自动识别并以 Base64 格式发送给支持视觉能力的 LLM（如 claude-sonnet-4-6、gpt-4.1）。
