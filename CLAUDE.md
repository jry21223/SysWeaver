...
├── agent/
│   ├── loop.rs          # ★ Agent 核心循环（TODO：需要实现）
│   ├── planner.rs       # 任务分解（单步/多步/消歧）
│   └── memory.rs        # 会话历史 + 操作记录 + Playbook
├── llm/
│   ├── client.rs        # Claude API 调用（tool_use 模式）
...
- 基础工具骨架：`shell.exec`、`file.read`、`system.info`
- LLM Client 骨架（`llm/client.rs`）
- System Prompt 构建（`llm/prompt.rs`）
- 系统环境扫描（`context/system_scan.rs`）
- CLI 入口（`main.rs`，命令解析完成，逻辑 TODO）

### 🚧 需要实现（按优先级）

**Phase 1（基础能力，对应 10 分）**
1. **`agent/loop.rs`** — Agent 核心循环（最关键）
   - Plan → Act（调用 LLM）→ Safety 校验 → 执行工具 → Observe → 反复
   - 支持最大 steps 限制（防止死循环）
   - 失败时自动重试（最多 2 次）

2. **`main.rs`** 中的 `run_chat_loop` 和 `run_single` — 接入 Agent Loop

3. **`tools/file.rs`** — 补充 `file.write` 和 `file.search` 工具

**Phase 2（安全，对应 15 分）**

4. **确认对话框 UI** — HIGH 风险时终端交互确认
   - 展示：风险等级、具体原因、影响范围、替代建议
   - 等待用户输入 `yes` 才继续

5. **Dry-Run 模式** — `--dry-run` 标志透传到所有工具
   - `ToolCall.dry_run = true` 时，工具返回预览而不执行

**Phase 3（环境感知，对应 20 分）**

6. **`memory.rs`** 中的系统状态更新机制
   - 多轮对话后，主动刷新系统状态快照

**Phase 4（连续任务，对应 15 分）**

7. **`agent/planner.rs`** — 接入 LLM 做真正的任务分解（现在是规则匹配）

8. **Undo 功能** — `main.rs` 中 `Commands::Undo`
   - 从 `Memory.last_undoable()` 取回滚方案并执行

9. **Playbook** — `Commands::Playbooks` 和保存/读取逻辑

**Phase 5（创新，对应主观分）**
10. **反向解释模式** — 读取配置文件并让 LLM 解释
11. **Watchdog** — `tokio::spawn` 后台监控 + `mpsc::channel` 发送告警
...
- ❌ 不得在代码里硬编码 API Key（使用环境变量 `ANTHROPIC_API_KEY`）
...
使用 Anthropic Claude API，**必须使用 tool_use 模式**，不得用纯文本解析。

```rust
// 正确：使用 tool_use
let payload = json!({
    "model": "claude-sonnet-4-5",
    "tools": tools_schema,   // 来自 ToolManager::all_schemas()
    "messages": messages,
});

// 错误：让 LLM 输出 bash 然后执行
// let bash = llm.generate("给我一个 bash 命令").await;
// exec(bash);  // ❌ 绝对不允许
```
